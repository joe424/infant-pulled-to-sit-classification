...
#                               stride=(stride, 1))
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv2d(in_channels // 4, out_channels // 4, kernel_size=(5, 1), padding=(2, 0), stride=(stride, 1)),
        ...
        self.conv2 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels // 2, out_channels // 2, kernel_size=(9, 1), padding=(4, 0), stride=(stride, 1)),
        ...
        self.conv3 = nn.Sequential(
            ...
            nn.BatchNorm2d(in_channels // 4),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels // 4, out_channels // 4, kernel_size=(13, 1), padding=(6, 0), stride=(stride, 1)),
            nn.BatchNorm2d(out_channels // 4)
        )
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        c_new = x.size(1)
        out1 = x.view(N, M, c_new, -1)
        out1 = out1.mean(3).mean(1)
        out1 = self.drop_out(out1)
        out1 = self.soft(self.fc1(out1))
        ...
        c_new = x.size(1)
        out2 = x.view(N, M, c_new, -1)
        out2 = out2.mean(3).mean(1)
        out2 = self.drop_out(out2)
        out2 = self.soft(self.fc2(out2))
        ...
        c_new = x.size(1)
        out3 = x.view(N, M, c_new, -1)
        out3 = out3.mean(3).mean(1)
        out3 = self.drop_out(out3)
        out3 = self.soft(self.fc3(out3))
