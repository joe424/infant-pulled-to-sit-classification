...
class atcn(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, embed_factor=3):
        ...
        self.conv_final = nn.Sequential(
            nn.Conv2d(embed_channels * 3, out_channels, kernel_size=1, groups=3),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(),
...
#         x4 = self.relu(self.conv4(x3))  # shape: N, out_channels, T, V
        ...
        out = torch.cat([x1, x2, x5], 1)  # shape: N, out_channels*5, T, V
...
class self_attention_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, coff_embedding=3, frames=300, num_subset=3):
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, stride=1, residual=True, frames=300):
        ...
        self.sagcn = self_attention_gcn(in_channels, out_channels, coff_embedding=3, frames=frames)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None,
                 graph_args=dict(), in_channels=3, out_channels=400, frames=300):
...
#         self.l3 = TCN_GCN_unit(out_channels // 2, out_channels, A, stride=2, frames=frames//4) ###
        ...
        self.l1 = TCN_GCN_unit(in_channels, out_channels, A, stride=2, residual=False, frames=frames) ###
