...
def parse_args():
    ...
    parser.add_argument('--cfg', help='experiment configure file name', required=False, type=str,
                        default="./lighttrack/dcpose/configs/posetimation/DcPose/posetrack18/model_HRnet.yaml")
...
#                         , default='./lighttrack/weights/epoch_45_state.pth') # 2021 model

#     parser.add_argument('-weight', help='model weight file', required=False, type=str
                        , default='./lighttrack/weights/epoch_37_state_optimize_HRNet.pth') # 2022 model
...
def get_inference_model():
    ...
    return new_model, cfg


model, cfg = get_inference_model()
model = model.cuda()
...
def inference_PE(input_image: str, prev_image: str, next_image: str, bbox):
    ...
    single = True
    if single: 
        target_image_data = image_preprocess(input_image, prev_image, next_image, center, scale)
    else:
    ...
    model.eval()
    predictions = model(target_image_data.cuda())#, margin=margin)
