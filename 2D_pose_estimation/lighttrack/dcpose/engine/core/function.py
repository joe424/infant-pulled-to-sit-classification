...
class Associative_loss(torch.nn.Module):
    """
     In a batch, every 2 person is a set containing either "2 same person" or "2 different person".
     i.e. in training iteration, batch size 80 actually has 40 original samples, 
          the other 40 samples is augmented which is paired to the original samples respectively.
    """
    def __init__(self, alpha=0.2, joint_size=17):
        super(Associative_loss, self).__init__()
        self.joint_size = joint_size
        self.alpha = alpha
    def forward(self, pred_tagmap, pred_depth_order, GT_heatmap_coord, target_heatmaps_weight, joint_vis=None):
        ...
        # target_heatmaps = joints_3d[batch, joint, 3(xy0)]
        semi_GT = pred_tagmap
        _, order_labels = torch.max(pred_depth_order, dim=1)
        ...
        inter_count = 0
        # iter through batch
        for b in range(0, len(order_labels), 2):
            set_relation = order_labels[b]
            coord_x1, coord_y1 = torch.tensor(GT_heatmap_coord[b,:,0]).long(), torch.tensor(GT_heatmap_coord[b,:,1]).long()
            weight1 = target_heatmaps_weight[b].view(-1) > 0
            # joint_filter = [False if (x==0 and y == 0) else True for x,y in zip(coord_x1, coord_y1)]
            check = True
            for i in weight1:
                if i == True:
                    check = False
            if check == True:
                ...
                continue
            inner_count += 1
            coord_x1, coord_y1 = coord_x1[weight1], coord_y1[weight1]
            
            tag1 = semi_GT[b, weight1,coord_y1, coord_x1]
            ref_coord1 = torch.mean(tag1, dim=0)
            if loss_inner == None:
                loss_inner = torch.mean((ref_coord1 - tag1)**2)
            else:
                loss_inner = loss_inner + torch.mean((ref_coord1 - tag1)**2)
            # If there is another person close to target
            if set_relation.item() is not 0:
                # print("set_relation", set_relation)
                # Inner loss of the augmented person
                coord_x2, coord_y2 = torch.tensor(GT_heatmap_coord[b+1,:,0]).long(), torch.tensor(GT_heatmap_coord[b+1,:,1]).long()
                weight2 = target_heatmaps_weight[b+1].view(-1) > 0
                check = True
                for i in weight2:
                    if i == True:
                        check = False
                ...
                inner_count += 1
                inter_count += 1
                coord_x2, coord_y2 = coord_x2[weight2], coord_y2[weight2]
                tag2 = semi_GT[b+1, weight2, coord_y2, coord_x2]

                ref_coord2 = torch.mean(tag2, dim=0)
                loss_inner =  loss_inner + torch.mean((ref_coord2 - tag2)**2)
                # print("ref_coord1", ref_coord1)
                # print("ref_coord2", ref_coord2)
                # inter loss
                # -1/(2*sigma**2)*(ref_coord1 - ref_coord2) **2)
                # print("torch.exp( -200 * (ref_coord1 - ref_coord2) **2)", torch.exp( -200 * (ref_coord1 - ref_coord2) **2))
                if loss_inter == None:
                    loss_inter = torch.exp( -200 * (ref_coord1 - ref_coord2) **2)
                else:
                    loss_inter = loss_inter + torch.exp( -200 * (ref_coord1 - ref_coord2) **2)
        if loss_inter != None:
            loss = loss_inner / inner_count + loss_inter/ inter_count
        else:
            loss = loss_inner / inner_count
        return loss
...
class FocalLoss(torch.nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        N = inputs.size(0)
        C = inputs.size(1)
        P = torch.nn.functional.softmax(inputs, dim=1)  # 先求p_t
        class_mask = inputs.data.new(N, C).fill_(0)
        class_mask = torch.autograd.Variable(class_mask)
        ids = torch.argmax(targets, dim=1)
...
class CommonFunction(BaseFunction):
    ...
    def train(self, model, epoch, optimizer, dataloader, tb_writer_dict, **kwargs):
        ...
        if self.PE_Name == "HRNETMH" or self.PE_Name == "HRNETMH_PRED_ASS" or self.PE_Name == "HRNETMH_PRED" or self.PE_Name == "HRNETGC_PRED":
            self.criterion_vis = FocalLoss()
        if self.PE_Name == "HRNETMH_PRED_ASS":
            self.criterion_ass = Associative_loss()
        logger = logging.getLogger(__name__)
        batch_time = AverageMeter()
        data_time = AverageMeter()
        losses = AverageMeter()
        acc = AverageMeter()
        acc_depth = AverageMeter()
        ...
        for iter_step in range(self.max_iter_num):
            if self.PE_Name == "DCPOSE" or self.PE_Name == "HRNET":
                input_x, input_sup_A, input_sup_B, target_heatmaps, target_heatmaps_weight, meta = next(self.dataloader_iter)
            elif self.PE_Name == "HRNETMH" or self.PE_Name == "HRNETMHV2" or self.PE_Name == "HRNETMHV2_H" or self.PE_Name == "HRNETMH_PRED_ASS" or self.PE_Name == "HRNETMH_PRED" or self.PE_Name == "HRNETGC_PRED":
                input_x, target_heatmaps, target_heatmaps_weight, meta = next(self.dataloader_iter)
            self._before_train_iter(input_x)

            data_time.update(time.time() - end)
            GT_heatmap_coord, _ = get_max_preds(target_heatmaps.numpy()) ###################
            ...
            if self.PE_Name == "HRNET":
            ...
            elif self.PE_Name == "HRNETMH" or self.PE_Name == "HRNETMHV2" or self.PE_Name == "HRNETMHV2_H":
                # vis_input = meta['joints_vis'].cuda()
                # vis_label = meta['depth_order'].cuda()
                lamba = meta['lamba'].cuda()
                # print("input_x",input_x.size())
                # print("lamba",lamba.size())
                outputs= model(input_x.cuda(), lamba=lamba)
            elif self.PE_Name == "HRNETMH_PRED" or self.PE_Name == "HRNETGC_PRED":
                vis_label = meta['order'].cuda(non_blocking=True)
                outputs, pred_vis= model(input_x.cuda(), depth_label=vis_label)
            elif self.PE_Name == "HRNETMH_PRED_ASS" :
                vis_label = meta['order'].cuda(non_blocking=True)
                joint_vis = meta['joints_vis'].cuda(non_blocking=True)
                outputs, pred_vis, pred_ass = model(input_x.cuda())
                ...
                if not self.cfg.LOSS.USE_SEMI_SUPERVISE:
                    outputs = model(concat_input, margin=margin)
                else:
                    outputs, patch_prev, patch_next, patch_cur = model(concat_input, margin=margin, target_heatmaps=target_heatmaps, target_heatmaps_weight=target_heatmaps_weight)
            ...
            if isinstance(outputs, list) or isinstance(outputs, tuple):
                ...
                loss = self.criterion(pred_heatmaps, target_heatmaps, target_heatmaps_weight)
                if self.PE_Name == "HRNETMH" or self.PE_Name == "HRNETMH_PRED" or self.PE_Name == "HRNETMH_PRED_ASS" or self.PE_Name == "HRNETGC_PRED":
                    depth_loss = 0.001 * self.criterion_vis(pred_vis, vis_label)
                    loss += depth_loss
                if self.PE_Name == "HRNETMH_PRED_ASS" :
                    embed_loss = 1e-3 * self.criterion_ass(pred_ass, vis_label, GT_heatmap_coord, target_heatmaps_weight)
                    loss += embed_loss
                    print("total_loss:{}, depth_loss:{}, embed_loss:{}".format(loss, depth_loss,embed_loss))
            # if self.cfg.LOSS.USE_SEMI_SUPERVISE:
            #     loss2 = 0.5 * (mse_loss_fun(patch_prev, patch_cur.detach()) + mse_loss_fun(patch_next, patch_cur.detach()))
            #     loss += loss2
            ...
            # avg_acc_depth, cnt_depth = depth_acc(pred_vis, vis_label)
            # acc_depth.update(avg_acc_depth, cnt_depth)
            ...
            if iter_step % self.cfg.PRINT_FREQ == 0 or iter_step >= self.max_iter_num - 1:
                print("Accuracy_depth {acc.val:.3f} ({acc.avg:.3f})".format(acc=acc_depth))
                msg = 'Epoch: [{0}][{1}/{2}]\t' \
    ...
    def eval(self, model, dataloader, tb_writer_dict, **kwargs):
        ...
        with torch.no_grad():
            ...
            for iter_step in range(self.max_iter_num):
                if self.PE_Name == 'DCPOSE' or self.PE_Name == 'HRNET':
                    input_x, input_sup_A, input_sup_B, target_heatmaps, target_heatmaps_weight, meta = next(self.dataloader_iter)
                elif self.PE_Name == 'HRNETMH' or self.PE_Name == 'HRNETMHV2' or self.PE_Name == "HRNETMHV2_H" or self.PE_Name == "HRnetMHV2_3L" or self.PE_Name == "HRNETMH_PRED_ASS" or self.PE_Name == "HRNETMH_PRED" or self.PE_Name == "HRNETGC_PRED":
                    input_x, target_heatmaps, target_heatmaps_weight, meta = next(self.dataloader_iter)
                # print(meta['image'])
                if phase == VAL_PHASE:
                ...
                if self.PE_Name == 'DCPOSE':
                    concat_input = torch.cat((input_x, input_sup_A, input_sup_B), 1).cuda()
                    outputs = model(concat_input, margin=margin)
                elif self.PE_Name == 'HRNETMH' or self.PE_Name == 'HRNETMHV2' or self.PE_Name == "HRNETMHV2_H" or self.PE_Name =="HRnetMHV2_3L" :
                    lamba = meta['lamba'].cuda()
                    outputs = model(input_x.cuda(), lamba=lamba)
                elif self.PE_Name == "HRNETMH_PRED" or self.PE_Name == "HRNETGC_PRED":
                    # vis_label = meta['order'].cuda(non_blocking=True)
                    outputs, _ = model(input_x.cuda())
                elif self.PE_Name == "HRNETMH_PRED_ASS":
                    # vis_label = meta['order'].cuda(non_blocking=True)
                    # joint_vis = meta['joints_vis'].cuda(non_blocking=True)
                    outputs, _, _ = model(input_x.cuda())
                    # print(outputs.size())
                else:
                    outputs = model(input_x.cuda())
                ...
                # post-processing for removing multi-hypothesis proposal
                if self.PE_Name == 'HRNETMH' or self.PE_Name == 'HRNETMHV2' or self.PE_Name == "HRNETMHV2_H" or self.PE_Name == "HRnetMHV2_3L" or self.PE_Name == "HRNETMH_PRED_ASS" or self.PE_Name == "HRNETMH_PRED":
                    from datasets.process.heatmaps_process import get_max_preds
                    mask_landa0 = [True if i %2 == 0 else False for i in range(len(meta['image']))]
                    mask_landa1 = [True if i %2 == 1 else False for i in range(len(meta['image']))]
                    # coords, maxvals  = get_max_preds(pred_heatmaps.detach().cpu().numpy())

                    # # calculate 
                    # ture_batch_size = int(1/2* pred_heatmaps.size(0))
                    # similarity = np.sum((coords[mask_landa0] - coords[mask_landa1])**2, axis=2).reshape(ture_batch_size, -1)
                    # similarity = np.sum(similarity, axis=1)
                    # sk_maxvals = np.sum(maxvals.reshape(pred_heatmaps.size(0), -1), axis=1)
                    # filterout_final = []
                    # for i in range(len(similarity)):
                    #     if similarity[i] < 5000:
                    #         if sk_maxvals[i] >= sk_maxvals[i+1]:
                    #             filterout_final.extend([True, False])
                    #         else: 
                    #             filterout_final.extend([False, True])
                    #     else:
                    #         filterout_final.extend([True, True])

                    ## filter
                    pred_heatmaps = pred_heatmaps[mask_landa0]

                    meta['image'] = list(compress(meta['image'], mask_landa0))
                    meta['center'] = meta['center'][mask_landa0]
                    meta['scale'] = meta['scale'][mask_landa0]
                    meta['score'] = meta['score'][mask_landa0]
                ...
                num_images = pred_heatmaps.size(0)
                # if self.PE_Name == 'HRNETMH':
                #     num_images = num_images*2
                # from .vis_helper import save_batch_heatmaps
                # print(meta['image'])
                # print(input_x.size())
                # heat_img_name = ["./out/"+osp.splitext(osp.basename(file_name))[0]+".jpg" for file_name in meta['image']]
                # save_batch_heatmaps(input_x.detach().cpu(), pred_heatmaps, heat_img_name[0], normalize=True)
    ...
    def _after_val_iter(self, image, preds_joints, preds_confidence, vis_output_dir, center, scale):
        ...
        if cfg.DEBUG.DEBUG:
            from .vis_helper import save_debug_images
            save_debug_images(config, input, meta, target, joints_pred, output, prefix)
