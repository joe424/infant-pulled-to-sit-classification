...
class DefaultRunner:
    ...
    def setup_cfg(self):
        ...
        is_fullset = cfg.DATASET.IS_FULLSET
        cfg.defrost()
        if is_fullset:
            cfg.OUTPUT_DIR = osp.join(cfg.OUTPUT_DIR, cfg.EXPERIMENT_NAME, dataset_name, "Fullset", hyper_parameters_setting)
        else:
            cfg.OUTPUT_DIR = osp.join(cfg.OUTPUT_DIR, cfg.EXPERIMENT_NAME, dataset_name, "Subset", hyper_parameters_setting)
        cfg.freeze()
