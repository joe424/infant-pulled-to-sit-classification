...
class PoseTrackMH(VideoDataset):
    """
        PoseTrackMH
    """

    def __init__(self, cfg, phase, **kwargs):
        super(PoseTrackMH, self).__init__(cfg, phase, **kwargs)
    ...
    def _get_spatiotemporal_window(self, data_item):
        ...
        # print(np.info(joints_vis[0]))
        # print(np.info(joints_vis[1]))
        center = data_item["center"]
        scale = data_item["scale"]
        lamba_ = data_item.get("lamba", None)
        if lamba_ is not None:
            lamba = torch.zeros((2,2))
            lamba_ = torch.tensor(lamba_)
            lamba = lamba.scatter_(1, lamba_, 1)
        else:
            lamba = torch.tensor([[1.,0.],[0.,1.]])
        order = data_item.get("order", None)

        score = data_item['score'] if 'score' in data_item else 1
        r = 0
        # print(self.image_size[0] , self.image_size[1] )
        # print("before_joints", joints)
        # print("before_joints_vis", joints_vis)
        half_body = False
        if self.is_train:
            for i in range(len(center)):
                if (np.sum(joints_vis[i][:, 0]) > self.num_joints_half_body and np.random.rand() < self.prob_half_body):
                    half_body = True
                    c_half_body, s_half_body = half_body_transform(joints[i], joints_vis[i], self.num_joints, self.upper_body_ids, self.aspect_ratio,
                                                                self.pixel_std)
                    center[i], scale[i] = c_half_body, s_half_body
            if half_body == True:
                order_ = torch.zeros((2,4)).float()
                order =  order_.scatter_(1,torch.tensor([[0],[0]]), 1.)
            ...
            for i in range(len(center)):
                scale[i] = scale[i] * np.clip(np.random.randn() * scale_factor + 1, 1 - scale_factor, 1 + scale_factor)
            ...
            if self.flip and random.random() <= 0.5:
                ...
                for i in range(len(center)):
                    joints[i], joints_vis[i] = fliplr_joints(
                        joints[i], joints_vis[i], data_numpy.shape[1], self.flip_pairs)
                    center[i][0] = data_numpy.shape[1] - center[i][0] - 1
        
        # calculate transform matrix
        input_x = []
        trans = []
        target_heatmaps = []
        target_heatmaps_weight = []
        for i in range(len(center)):
            trans.append(get_affine_transform(center[i], scale[i], r, self.image_size))
            input_x.append(cv2.warpAffine(np.copy(data_numpy), trans[i], (int(self.image_size[0]), int(self.image_size[1])), flags=cv2.INTER_LINEAR))
            cv2.imwrite('output{}_{}.jpg'.format(osp.splitext(osp.basename(image_file_path[i]))[0],i), input_x[i])
            if self.transform:
                input_x[i] = self.transform(input_x[i])
            
            # joint transform like image
            for j in range(self.num_joints):
                if joints_vis[i][j, 0] > 0.0:  # joints_vis   num_joints  x 3 (x_vis,y_vis)
                    joints[i][j, 0:2] = exec_affine_transform(joints[i][j, 0:2], trans[i])
                
            # H W
            for index, joint in enumerate(joints[i]):
                x, y, _ = joint
                if x < 0 or y < 0 or x > self.image_size[0] or y > self.image_size[1]:
                    joints_vis[i][index] = [0, 0, 0]

        # target_heatmaps, target_heatmaps_weight = self._generate_target(joints, joints_vis, self.heatmap_size, self.num_joints)

            target_heatmaps_, target_heatmaps_weight_ = generate_heatmaps(joints[i], joints_vis[i], self.sigma, self.image_size, self.heatmap_size,
                                                                        self.num_joints,
                                                                        use_different_joints_weight=self.use_different_joints_weight,
                                                                        joints_weight=self.joints_weight)
            target_heatmaps_ = torch.from_numpy(target_heatmaps_)  # H W
            target_heatmaps_weight_ = torch.from_numpy(target_heatmaps_weight_)

            target_heatmaps.append(target_heatmaps_)
            target_heatmaps_weight.append(target_heatmaps_weight_)

        meta = {
            'image': [image_file_path,image_file_path],
            'filename': [filename, filename],
            'imgnum': [img_num, img_num],
            'joints': joints,
    ...
    def collate_fn(self, batch):
        variable_batch_size = 0
        for j, b in enumerate(batch):
            variable_batch_size += len(b[0]) # list
            if j == 0:
                input_x_new = torch.cat(b[0], 0)
                target_heatmaps_new = torch.cat(b[1], 0)
                target_heatmaps_weight_new = torch.cat(b[2], 0)
            else:
                _input_x_new = torch.cat(b[0], 0)
                _target_heatmaps_new = torch.cat(b[1], 0)
                _target_heatmaps_weight_new = torch.cat(b[2], 0)

                input_x_new = torch.cat((input_x_new, _input_x_new), 0)
                target_heatmaps_new = torch.cat((target_heatmaps_new, _target_heatmaps_new), 0)
                target_heatmaps_weight_new = torch.cat((target_heatmaps_weight_new, _target_heatmaps_weight_new), 0)
  
            meta = b[-1]
            if j == 0:
             meta_new = meta
            else:
                meta_new['image'].extend(meta['image'])
                meta_new['filename'].extend(meta['filename'])
                meta_new['imgnum'].extend(meta['imgnum'])
                meta_new['joints'].extend(meta['joints'])
                meta_new['joints_vis'].extend(meta['joints_vis'])                        
                meta_new['center'].extend(meta['center'])                        
                meta_new['scale'].extend(meta['scale'])                        
                meta_new['rotation'].extend(meta['rotation'])                        
                meta_new['score'].extend(meta['score'])  
                meta_new['lamba'].extend(meta['lamba'])  
                meta_new['order'].extend(meta['order'])  

        # To tensor
        meta_new['joints'] = torch.from_numpy(np.concatenate(meta_new['joints'], 0))     
        meta_new['joints_vis'] = torch.from_numpy(np.concatenate(meta_new['joints_vis'], 0))
        meta_new['center'] = torch.from_numpy(np.concatenate(meta_new['center'], 0))              
        meta_new['scale'] = torch.from_numpy(np.concatenate(meta_new['scale'], 0))  
        meta_new['rotation'] = torch.tensor(meta_new['rotation'])
        meta_new['score'] = torch.tensor(meta_new['score'])
        meta_new['lamba'] = torch.cat(meta_new['lamba'], 0) 
        if meta_new['order'][0] is not None:
            meta_new['order'] = torch.cat(meta_new['order'], 0) 
        # print(meta_new['score'].size())
        # print(meta_new['rotation'].size())
        # print(type(meta_new['lamba']))
        meta_new['joints'] =  meta_new['joints'].view(variable_batch_size, self.num_joints,3)
        meta_new['joints_vis'] =meta_new['joints_vis'].view(variable_batch_size, self.num_joints,3)
        meta_new['center'] = meta_new['center'].view(variable_batch_size, 2)
        meta_new['scale'] = meta_new['scale'].view(variable_batch_size, 2)
        input_x_new = input_x_new.view(variable_batch_size, 3, input_x_new.size(1), input_x_new.size(2))
        target_heatmaps_new = target_heatmaps_new.view(variable_batch_size, self.num_joints, target_heatmaps_new.size(1), target_heatmaps_new.size(2))
        target_heatmaps_weight_new = target_heatmaps_weight_new.view(variable_batch_size,self.num_joints, target_heatmaps_weight_new.size(1))

        return input_x_new, target_heatmaps_new, target_heatmaps_weight_new, meta_new
    ...
    def _load_coco_keypoints_annotations(self):
        ...
        for index in self.image_set_index:
            ...
            for obj_index, obj in enumerate(objs):
                ...
                is_group_crowded_person = False
                if min_val[obj_index] < distance_far:
                    is_group_crowded_person = True
                    idx = min_idx[obj_index]
                    # print("{}, min_idx[obj_index]{}".format(obj_index, min_idx[obj_index]))
                    obj2 = objs[idx]
                    order_label = multi_person_relative_depth_list[[obj_index, int(idx.item())]].long().unsqueeze(1)
                    # print("order_label", order_label)
                    # print("{}, min_idx[obj_index]{} , order={}".format(obj_index, min_idx[obj_index], order_label))
                    order = torch.zeros((2,4)).float()
                    order = order.scatter_(1,order_label, 1.)

                    
                if is_group_crowded_person == True:
                    joints_3d = np.zeros((self.num_joints, 3), dtype=np.float)
                    joints_3d_vis = np.zeros((self.num_joints, 3), dtype=np.float)
                    joints_3d2 = np.zeros((self.num_joints, 3), dtype=np.float)
                    joints_3d_vis2 = np.zeros((self.num_joints, 3), dtype=np.float)
                    for ipt in range(self.num_joints):
                        ...
                        joints_3d2[ipt, 0] = obj2['keypoints'][ipt * 3 + 0]
                        joints_3d2[ipt, 1] = obj2['keypoints'][ipt * 3 + 1]
                        joints_3d2[ipt, 2] = 0
                        t_vis2 = obj2['keypoints'][ipt * 3 + 2]
                        ...
                        if t_vis2 > 1:
                            t_vis2 = 1
                        joints_3d_vis2[ipt, 0] = t_vis2
                        joints_3d_vis2[ipt, 1] = t_vis2
                        joints_3d_vis2[ipt, 2] = 0

                    center, scale = box2cs(obj['clean_bbox'][:4], self.aspect_ratio, self.bbox_enlarge_factor)
                    center2, scale2 = box2cs(obj2['clean_bbox'][:4], self.aspect_ratio, self.bbox_enlarge_factor)

                    rec.append({
                        'image': osp.join(self.img_dir, file_name),
                        'center': [center, center2],
                        'scale': [scale, scale2],
                        'box': [obj['clean_bbox'][:4], obj2['clean_bbox'][:4]],
                        'joints_3d': [joints_3d, joints_3d2],
                        'joints_3d_vis': [joints_3d_vis, joints_3d_vis2],
                        'filename': '',
                        'imgnum': [0],
                        'nframes': [nframes, nframes],
                        'frame_id': [frame_id, frame_id],
                        'is_group_crowded_person': [True, True],
                        'lamba': [[0],[1]],
                        'order': order,
                    })
                else:            
                    ...
                    rec.append({
                        'image': osp.join(self.img_dir, file_name),
                        'center': [center, np.copy(center)],
                        'scale': [scale, np.copy(scale)],
                        'box': [obj['clean_bbox'][:4], np.copy(obj['clean_bbox'][:4])],
                        'joints_3d': [joints_3d, np.copy(joints_3d)],
                        'joints_3d_vis': [joints_3d_vis, np.copy(joints_3d_vis)],
                        'filename': '',
                        'imgnum': 0,
                        'nframes': [nframes,nframes],
                        'frame_id': [frame_id, frame_id],
                        'is_group_crowded_person': [False,False],
                        'lamba': [[0],[1]],
                        'order':order,
                    })
            gt_db.extend(rec)
