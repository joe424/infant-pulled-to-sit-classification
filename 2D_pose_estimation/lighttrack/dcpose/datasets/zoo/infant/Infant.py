...
class Infant(VideoDataset):
    """
        Infant
    """

    def __init__(self, cfg, phase, **kwargs):
        super(Infant, self).__init__(cfg, phase, **kwargs)
        ...
        self.frame_seq_digits = 4
        self.is_posetrack18 = cfg.DATASET.IS_POSETRACK18
        self.transform = build_transforms(cfg, phase)

        self.distance_whole_otherwise_segment = 2#cfg.DISTANCE_WHOLE_OTHERWISE_SEGMENT
        self.distance =  2#cfg.DISTANCE
        ...
        self.coco = COCO(osp.join(self.json_dir, 'train_0412PTS_5sec.json' if self.is_train else 'val_0412PTS_5sec.json'))
    ...
    def _get_spatiotemporal_window(self, data_item):
        ...
        filename_seq = osp.basename(filename).replace('.jpg', '')[-4:]
        filename_seq_num = int(filename_seq)
        if filename_seq_num == 0: 
            is_start = True
        current_idx = int((filename_seq_num - beg_seq) / 5)
        

        ##TODO##
        if self.distance_whole_otherwise_segment:
            farthest_distance = self.distance
            prev_delta_range = range(0, min(current_idx, farthest_distance))
            next_delta_range = range(0, min((num_frames - current_idx), farthest_distance))
        else:
            prev_delta_range = range(0, min(current_idx , self.previous_distance))
            next_delta_range = range(0, min((num_frames - current_idx) ,self.next_distance))

        prev_delta_range = list(prev_delta_range)
        next_delta_range = list(next_delta_range)
        # print("prev_delta_range",prev_delta_range)
        # print("next_delta_range",next_delta_range)
        ##TODO##
        ...
        prev_idx = (current_idx - prev_delta) * 5
        next_idx = (current_idx + next_delta) * 5

        prev_seq = filename_seq_num - prev_idx
        next_seq = filename_seq_num + next_idx

        prev_img_name = filename.replace(filename_seq, str(prev_seq).zfill(self.frame_seq_digits) )
        next_img_name = filename.replace(filename_seq, str(next_seq).zfill(self.frame_seq_digits) )
        prev_image_file = osp.join(osp.dirname(image_file_path), prev_img_name)
        next_image_file = osp.join(osp.dirname(image_file_path), next_img_name)
        ...
        return input_x, -1, -1,  target_heatmaps, target_heatmaps_weight, meta  #input_prev, input_next,
    ...
    def _load_coco_keypoints_annotations(self):
        ...
        for index in self.image_set_index:
            ...
            dataset_id = im_ann['dataset_id']
            frame_bg2end = self.fm_bg2end_dict.get(dataset_id)
            if frame_bg2end == None:
                clip_dataset = [fm for fm in self.image_set_index if self.coco.loadImgs(fm)[0]['dataset_id'] == dataset_id]
                num_dataset = len(clip_dataset)
                video_name = self.coco.loadImgs(clip_dataset[0])[0]['file_name']
                self.fm_bg2end_dict[dataset_id] = [video_name, num_dataset]
            nframes = self.fm_bg2end_dict[dataset_id][1]
            # nframes = int(im_ann['id'])
            ...
            # print(clip_dataset)
            # print("self.coco.loadImgs(clip_dataset[0])[0]['dataset_id']", self.coco.loadImgs(clip_dataset[0])[0]['file_name'])
            beg_seq = int(self.coco.loadImgs(clip_dataset[0])[0]['file_name'].replace('.jpg', '')[-4:])
    ...
    def image_path_from_index(self, index):
        img_ann = self.coco.loadImgs(index)
        img_name = osp.basename(img_ann[0]["path"])
        img_name = img_name.split("_")
        img_name[1] = img_name[1][:img_name[1].find("m")+1]
        img_name = "_".join(img_name)

        clip_dir = img_ann[0]["path"].split('/')[2]
        clip_dir = clip_dir.split("_")
        clip_dir[1] = clip_dir[1][:clip_dir[1].find("m")+1]
        clip_dir = "_".join(clip_dir)

        video_dir = clip_dir.rsplit('_', 3)[0]
        # video_dir = re.sub('*[1-9]m*_')video_dir.replace()
        img_split = img_name.split('_')
        
        ca_age = img_split[1]
        ca_age = ca_age[:ca_age.find("m")]
        ca_age_new = ca_age.split(".")[0]

        if ca_age_new == ca_age:
            ca_age_new = "CA" + ca_age_new + "m"
        else:
            ca_age_new = "CA" + ca_age_new + "m"
        # "26_4m_Prone_3_0185.jpg" would be like [ 26, 4m, Prone, 3, 0185.jpg]
        
        image_path = osp.join(self.img_dir, 
                                  img_split[0], 
                                  ca_age_new,
                                  video_dir,
                                  clip_dir,
    ...
    def evaluate(self, cfg, preds, output_dir, boxes, img_path, *args, **kwargs):
        ...
        for key in img_path:
            ...
            video_name = temp[-3] + '/' + temp[-2]
            print("video_name", video_name)
            filename_seq = osp.basename(video_name).replace('.jpg', '')[-4:]
            img_sfx = temp[-3] + '/' + temp[-2] + '/' + temp[-1]
            print("img_sfx", img_sfx)
            prev_nm = temp[len(temp) - 1]
            frame_num = int(prev_nm.replace('.jpg', '')[-4:])
            if not video_name in video_map:
                video_map[video_name] = [cc]
                vid2frame_map[video_name] = [frame_num]
                vid2name_map[video_name] = [img_sfx]
            else:
                video_map[video_name].append(cc)             # cc
                vid2frame_map[video_name].append(frame_num)  # frame_seq
                vid2name_map[video_name].append(img_sfx)     # image_name 
        ...
        out_filenames, L = self.fm_bg2end_dict[:][0], []
