...
# parser.add_argument("--weights_path", type=str, default="/home/mbl/HDD_disk/infant_video/pose_estimation_weights/weights/YOLOv3/yolov3_ckpt80.pth", help="path to weights file")

# parser.add_argument("--weights_path", type=str, default="./weights/yolov3_ckpt80.pth", help="path to weights file") #### senior

# parser.add_argument("--weights_path", type=str, default="/home/mbl/HDD_disk/Kuei/PyTorch-YOLOv3/checkpoints/yolov3_ckpt_300.pth", help="path to weights file") ####

parser.add_argument("--weights_path", type=str, default="./weights/yolov3_ckpt161_subdiv_4_lr_0_0001_dev_0.pth", help="path to weights file") #### optimize

parser.add_argument("--conf_thres", type=float, default=0.3, help="object confidence threshold") #### 原本0.4，0602改成0.3
...
#         detections = non_max_suppression(detections, 0.15, opt.nms_thres)[0] ####
        detections = non_max_suppression(detections, opt.conf_thres, opt.nms_thres)[0] ####
        
#         sub = 0.05 ####
#         _detections = model(input_img) ####
#         detections = non_max_suppression(_detections, opt.conf_thres, opt.nms_thres)[0] ####
#         while (detections is None) and (opt.conf_thres - sub >= 0.15): ####
#             detections = non_max_suppression(_detections, opt.conf_thres - sub, opt.nms_thres)[0] ####
#             sub += 0.05
...
#             if int(cls_pred) == 0 and cls_conf > best_cls_conf: ####
#                 best_cls_conf = cls_conf ####
#                 human_candidates = [x1, y1, box_w, box_h] ####
#     if len(human_candidates) < 4: ####
#         return [] ####
#     else: ####
