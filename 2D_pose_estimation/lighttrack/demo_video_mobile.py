...
sys.path.append('./HPE/')
# detector utils
print("detector_yolov3_BEGIN")
from detector.detector_yolov3 import *
print("detector_yolov3")
...
from utils_io_folder import *
def show_boxes_from_python_data(name,img_path, img, det, output_img_path, scale = 1.0):
    ...
    output_img_pth = output_img_dir + "/" + os.path.basename(img_path)
    print(output_img_pth)
    plt.cla()
    plt.axis("off")
    plt.imshow(img)
...
def initialize_parameters():
    ...
    keyframe_interval = 3 # choice examples: [2, 3, 5, 8, 10, 20, 40, 100, ....]
...
def light_track(pose_estimator,
    ...
    while img_id < num_imgs-1:
        ...
        print("Current tracking: [image_id:{}], path:{}".format(img_id, img_path))
        ...
        if is_keyframe(img_id, keyframe_interval) or flag_mandatory_keyframe:
            ...
            for det_id in range(num_dets):
                ...
                print("key_frame:BBX", bbox_det)
                show_boxes_from_python_data("key_frame",img_path, img, bbox_det, output_img_path=0, scale = 1.0)
                ####
            ...
            num_dets_yolo = len(human_candidates)

            print("Keyframe: {} detections".format(num_dets_yolo))
            if num_dets_yolo == 0 and num_dets == 0:
            # if num_dets == 0:
                flag_mandatory_keyframe = True

            for det_id in range(num_dets):
                keypoints = keypoints_list[det_id]["keypoints"]
                if num_dets_yolo != 0 and det_id < num_dets_yolo:
                    bbox_det = human_candidates[det_id]
                    # enlarge bbox by 20% with same center position
                    bbox_x1y1x2y2 = xywh_to_x1y1x2y2(bbox_det)
                    bbox_in_xywh = enlarge_bbox(bbox_x1y1x2y2, enlarge_scale)
                    ...
                    bbox_det_dict = {"img_id":img_id,
                                     ...
                                     "bbox":bbox_det}
                    keypoints = inference_keypoints(pose_estimator, bbox_det_dict)[0]["keypoints"]
                print("keypoints of current track:", keypoints)
                ...
                if target_lost is False:
                    ...
                    ###
                    print("track_frame:BBX", bbox_det_next)
                    img = np.array(Image.open(img_path).convert('RGB'), dtype=np.uint8)
                    show_boxes_from_python_data("track_frame",img_path, img, bbox_det_next, output_img_path=0, scale = 1.0)
                    ####
...
def enlarge_bbox(bbox, scale):
    ...
    print("before_enlarge_bbox:[{},{},{},{}]".format(min_x,min_y, max_x, max_y))
    ...
    min_y -= margin_y
    max_y += margin_y
    print("enlarged_bbox:[{},{},{},{}]".format(min_x,min_y, max_x, max_y))
    width = max_x - min_x
    height = max_y - min_y
...
def get_keypoints_from_pose(pose_heatmaps, details, cls_skeleton, crops, start_id, end_id):
    ...
    for test_image_id in range(start_id, end_id):
        ...
        print("keypoints of next track:", cls_skeleton)
    return cls_skeleton
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video_dir', '-v', type=str, dest='video_dir', default="/home/mbl/HDD_disk/infant_video/實驗收案/HPE_dataset/Pull_to_sit_20210412/")
    parser.add_argument('--model', '-m', type=str, dest='test_model', default= "logs/default.HPE/model_dump_MSRA152COCO/snapshot_20.ckpt")  #   "weights/mobile-deconv/snapshot_296.ckpt")
    ...
    data_root = "/home/mbl/HDD_disk/infant_video/實驗收案/HPE_dataset/Pull_to_sit_20210412/"
    visualize_folder_root = data_root + "out/visualize/"
    output_video_folder_root = data_root + "out/videos/"
    output_json_folder_root = data_root + "out//jsons/"


    videos_path = glob.glob(video_dir+"[1-9][0-9]/*/*")
    # for root, dirs, files in os.walk(video_dir):
    skip = True
    for video_path in videos_path:
        print("Current video_path: ", video_path)
        
        v_dir, video_name = os.path.split(video_path)
        # print(video_name)
        # video_name = video_name.replace(" ", "_")[:-4]
        
        image_folder = video_path + "/"

        v_dir_mid = v_dir[10:] + "/"
        output_json_dir = os.path.abspath(os.path.join(output_json_folder_root))
        output_video_dir = os.path.abspath(os.path.join(output_video_folder_root))

        visualize_folder = os.path.abspath(visualize_folder_root +  video_name)
        output_json_path = os.path.join(output_json_dir, video_name+".json")
        output_video_path = os.path.join(output_video_dir, video_name+"_out.mp4")
        
        # /home/mbl/HDD_disk/infant_video/實驗收案/HPE_dataset/Pull_to_sit_20210412_5sec/out/visualize/9_4m_Pull_to_sit_3_5secClip_5frameSkip_1
        if (not "10_6m_Pull_to_sit_1" in video_path) and skip == True:
            skip = False
            continue
        ...
        light_track(pose_estimator,image_folder, output_json_path,
                    visualize_folder, output_video_path)
