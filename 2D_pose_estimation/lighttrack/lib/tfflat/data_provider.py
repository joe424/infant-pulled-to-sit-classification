    ...
    def get_data(self):
        if self._is_train:
            ...
            if self._shuffle:
                self.rng.shuffle(idxs)
            for i in idxs:
                # print("DATAFROMLIST:self._datalist[i]: ", self._datalist[i])
                yield self._datalist[i]
...
class MultiProcessMapDataZMQ(_ParallelMapData):
    ...
    def _send(self, dp):
        ...
        print("# round-robin assignment, MultiProcessMapDataZMQ")
    ...
    def get_data(self):
        print("get_data in MultiProcessMapData")
...
class BatchData(object):
    ...
    def get_data(self):
        ...
        for data in self.ds.get_data():
            holder.append(data)
            # print("BatchData:get_data")
            if len(holder) == self.batch_size:
                # print("BatchData:get_data:len(holder) == self.batch_size")
    ...
    def _aggregate_batch(data_holder, use_list=False):
        ...
        # print("BatchData:_aggregate_batch:size", size)
        result = []
        for k in range(size):
            if use_list:
                result.append(
